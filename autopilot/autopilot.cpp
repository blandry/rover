...
Autopilot::Autopilot() {
    ...
    ioboard = serialport_init("/dev/ttyACM0", 115200);
    ...
    mast_setpoint.mast_pitch = 1500;
    mast_setpoint.mast_yaw = 1500;
}
...
void Autopilot::execute_command() {
    ...
    // sends the command to the mast
    ROS_INFO("yaw=%i pitch=%i", mast_setpoint.mast_yaw, mast_setpoint.mast_pitch);
    send_pwm(12, mast_setpoint.mast_yaw);
    send_pwm(13, mast_setpoint.mast_pitch);
}
...
void Autopilot::rover_cmds_callback(const std_msgs::String::ConstPtr& msg) {
    ...
    unsigned int default_mast_inc_pitch = 40;
    unsigned int default_mast_inc_yaw = 40;
    ...
    } else if (strcmp(cmd,"MAST-PITCHUP")==0) {
        if (mast_setpoint.mast_pitch < 2200) {
            mast_setpoint.mast_pitch += default_mast_inc_pitch;
        }
    } else if (strcmp(cmd,"MAST-PITCHDOWN")==0) {
        if (mast_setpoint.mast_pitch > 800) {
            mast_setpoint.mast_pitch -= default_mast_inc_pitch;
        }
    } else if (strcmp(cmd,"MAST-YAWLEFT")==0) {
        if (mast_setpoint.mast_yaw < 2200) {
            mast_setpoint.mast_yaw += default_mast_inc_yaw;
        }
    } else if (strcmp(cmd,"MAST-YAWRIGHT")==0) {
        if (mast_setpoint.mast_yaw > 800) {
            mast_setpoint.mast_yaw -= default_mast_inc_yaw;
        }
    }
}
